...
export function useTerminalCommand(extensionContext: ExtensionContext): {
  ...
  async function executeCommandWithResult(
    command: string,
    options: TerminalExecutionOptions = {}
  ): Promise<CommandResult> {
    const { requireConfirmation, captureOutput = true } = options;
