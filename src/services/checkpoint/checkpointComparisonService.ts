...
export class CheckpointComparisonService implements vscode.Disposable {
  ...
  public async compareCheckpoints(
    ...
    const fromChanges =
      fromCheckpointId === 'current' ? currentWorkspaceState : fromCheckpoint?.changes || [];

    const toChanges =
      toCheckpointId === 'current' ? currentWorkspaceState : toCheckpoint?.changes || [];
  ...
  public async visualizeDiff(
    ...
    try {
      const comparison = await this.compareCheckpoints(fromCheckpointId, toCheckpointId, options);
      ...
      panel.webview.onDidReceiveMessage(async (message) => {
        if (message.command === 'openDiff' && message.filePath) {
          await this.openFileDiff(message.filePath, fromCheckpointId, toCheckpointId);
