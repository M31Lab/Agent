...
export class CodebaseUnderstandingService implements ICodebaseUnderstandingService, vscode.Disposable {
    ...
    public async analyzeFullCodebase(): Promise<CodebaseAnalysisResult> {
        ...
        if (
            ...
            currentTimestamp - this.lastFullAnalysisTimestamp < CodebaseUnderstandingService.CACHE_EXPIRY_MS
        ) {
        ...
        try {
            ...
            const complexityHotspots = await this.findComplexityHotspots(baseAnalysis.files.map(f => f.path));
    ...
    public async generateCodebaseOverview(): Promise<CodebaseOverview> {
        try {
            ...
            const projectName = mainPackageJson?.name || path.basename(vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || '');
            const buildSystem = this.determineBuildSystem(analysis.files.map(f => f.path));
            const projectType = this.determineProjectType(analysis.files.map(f => f.path), mainPackageJson);
            const entryPoints = this.findEntryPoints(analysis.files.map(f => f.path), projectType);
                    ...
                    if (importUri) {
                        ...
                        for (const symbol of importSymbols) {
                            if (symbol.name.startsWith(word)) {
                                const item = new vscode.CompletionItem(symbol.name, this.symbolKindToCompletionItemKind(symbol.kind));
        ...
        try {
            ...
            for (const file of analysis.files) {
                try {
                    ...
                    this.context.loggingService.error(`Error analyzing file for task relevance: ${file.path}`, error);
                }
            }
    ...
    public async getSymbolUsageAnalysis(symbolName: string): Promise<SymbolUsageAnalysis> {
        try {
            ...
            for (const location of locations) {
                ...
                const isModification = this.isSymbolModified(line.text, symbolName, location.range.start.character);
            ...
            const usagePatterns = this.analyzeUsagePatterns(symbolName, usageLocations, modificationLocations);
        ...
        try {
            ...
            for (const file of analysis.files) {
                try {
                    ...
                    this.context.loggingService.error(`Error building dependency graph for ${file.path}`, error);
                }
            }
    ...
    public async getModuleInsights(modulePath: string): Promise<ModuleInsights> {
        try {
            const directory = path.isAbsolute(modulePath) ? modulePath : path.join(vscode.workspace.workspaceFolders![0].uri.fsPath, modulePath);
            ...
            for (const file of files) {
                try {
                    ...
                    this.context.loggingService.error(`Error analyzing file for module insights: ${file}`, error);
                }
            }
        ...
        try {
            ...
            for (const file of analysis.files) {
                try {
                    ...
                    const functionRegex = new RegExp(`\\b(function|async\\s+function|const|let|var)\\s+${functionName}\\b|\\b${functionName}\\s*=\\s*(async\\s*)?\\([^)]*\\)\\s*=>|\\bclass\\s+${functionName}\\b`, 'g');
            ...
            this.context.loggingService.error(`Failed to find function definition for ${functionName}`, error);
            throw error;
        }
    }
        ...
        try {
            ...
            for (const file of analysis.files) {
                try {
                    ...
                    let changeFrequency = 0;
                    try {
                        const gitExecResult = await vscode.commands.executeCommand<{stdout: string, stderr: string}>(
                        ...
                        if (gitExecResult && gitExecResult.stdout) {
                            changeFrequency = gitExecResult.stdout.split('\n').filter(line => line.trim().length > 0).length;
                        }
                    } catch (error) {
    ...
    private async findComplexityHotspots(filePaths: string[]): Promise<ComplexityHotspot[]> {
        ...
        for (const filePath of filePaths) {
            try {
                ...
                const functionRegex = /function\s+\w+\s*\([^)]*\)\s*{|(\w+)\s*[=:]\s*(?:async\s*)?\([^)]*\)\s*(?:=>)?\s*{|\bclass\s+(\w+)(?:\s+extends\s+\w+)?\s*{/g;
                let match;
                ...
                const complexConditionRegex = /if\s*\([^()]*&&[^()]*&&[^()]*\)|if\s*\([^()]*\|\|[^()]*\|\|[^()]*\)/g;
        ...
        try {
            ...
            for (const packageJsonUri of packageJsonFiles) {
                try {
                    ...
                    this.context.loggingService.error(`Error parsing package.json: ${packageJsonUri.fsPath}`, error);
                }
            }
                    ...
                    try {
                        ...
                        for (const dependency of dependencies) {
                            ...
                            const importRegex = new RegExp(
                                `(?:import|require)\\s*\\(?[^)]*['"]${dependency.name}(?:/.+)?['"]`, 'g'
                            );
    ...
    private determineProjectType(filePaths: string[], packageJson: unknown[] | null): string {
        ...
        if (fileExtensions.includes('.cs') && fileNames.includes('program.cs')) return '.NET Console App';
        if (fileExtensions.includes('.cs') && fileNames.some(f => f.includes('controller'))) return 'ASP.NET App';
    ...
    private async extractImports(document: vscode.TextDocument): Promise<string[]> {
        ...
        try {
            ...
            this.context.loggingService.error(`Error extracting imports from ${document.uri.fsPath}`, error);
        }
        ...
        try {
            ...
            for (const calledFunction of calls) {
                try {
                    ...
                    this.context.loggingService.error(`Error processing function call: ${calledFunction}`, error);
                }
            }
        } catch (error) {
        ...
        try {
            ...
            if (['javascript', 'typescript', 'javascriptreact', 'typescriptreact'].includes(languageId)) {
                ...
                while ((match = varDeclarationRegex.exec(functionText)) !== null) {
                    ...
                    const endPos = document.positionAt(functionRange.start.character + match.index + match[0].length);
                    ...
                    while ((usageMatch = usageRegex.exec(functionText)) !== null) {
                        ...
                        const usageStartPos = document.positionAt(functionRange.start.character + usageMatch.index);
                        const usageEndPos = document.positionAt(functionRange.start.character + usageMatch.index + varName.length);
                        usages.push(new vscode.Location(document.uri, new vscode.Range(usageStartPos, usageEndPos)));
                    }
                ...
                if (headerMatch) {
                    ...
                    for (let i = 0; i < params.length; i++) {
                        ...
                        const endPos = document.positionAt(functionRange.start.character + paramIndex + param.length);
                        const location = new vscode.Location(document.uri, new vscode.Range(startPos, endPos));
                        ...
                        while ((usageMatch = usageRegex.exec(functionText)) !== null) {
                            ...
                            const usageStartPos = document.positionAt(functionRange.start.character + usageMatch.index);
                            const usageEndPos = document.positionAt(functionRange.start.character + usageMatch.index + paramName.length);
                            usages.push(new vscode.Location(document.uri, new vscode.Range(usageStartPos, usageEndPos)));
                        }
            ...
            for (const path of paths) {
                ...
                for (const node of path.path) {
                    try {
                        ...
                        this.context.loggingService.error(`Error analyzing conditions in ${node.function}`, error);
                    }
                }
    ...
    private async extractExports(document: vscode.TextDocument): Promise<string[]> {
        ...
        try {
            ...
            if (['javascript', 'typescript', 'javascriptreact', 'typescriptreact'].includes(languageId)) {
                // Named exports
                const namedExportRegex = /export\s+(?:const|let|var|function|class|interface|type|enum)\s+([a-zA-Z_$][\w$]*)/g;
                let match;
            ...
            // Java/C# public classes and methods
                ...
                // Public methods
                const methodRegex = /public\s+(?:static\s+)?(?:[a-zA-Z_<>][\w<>]*)\s+([a-zA-Z_][\w]*)\s*\(/g;
            ...
            this.context.loggingService.error(`Error extracting exports from ${document.uri.fsPath}`, error);
        }
    ...
    private inferModulePurpose(
        ...
        if (fileExts.some(e => e === '.test.js' || e === '.test.ts' || e === '.spec.js' || e === '.spec.ts')) return 'Tests';
    ...
    private async detectPatterns(filesByFolder: Record<string, string[]>): Promise<unknown[][]> {
        ...
        const folderNames = Object.keys(filesByFolder).map(folder => path.basename(folder).toLowerCase());
        ...
        if (
            folderNames.includes('containers') &&
        ...
        for (const folder in filesByFolder) {
            for (const file of filesByFolder[folder]) {
                try {
                    ...
                    this.context.loggingService.error(`Error analyzing file for factory pattern: ${file}`, error);
                }
            }
        }
    ...
    private detectArchitecturalLayers(filesByFolder: Record<string, string[]>): string[] {
        ...
        const possibleLayers = [
            ...
            'data', 'repository', 'dao', 'storage',
            'infrastructure', 'services', 'utils', 'helpers',
            'api', 'network', 'remote'
        ];
        ...
        for (const folder in filesByFolder) {
            ...
            for (const file of filesByFolder[folder]) {
                try {
                    ...
                    for (const importPath of imports) {
                        ...
                        for (const targetComponent of components) {
                            ...
                            if (importPath.includes(targetComponent)) {
                                ...
                                if (importPath.toLowerCase().includes('interface') || importPath.includes('contract')) {
                    ...
                    this.context.loggingService.error(`Error detecting component relationships: ${file}`, error);
                }
            }
        }
    ...
    private async calculateFileComplexity(document: vscode.TextDocument): Promise<number> {
        try {
            ...
            const loops = (text.match(/for\s*\(/g) || []).length + (text.match(/while\s*\(/g) || []).length;
            ...
            this.context.loggingService.error(`Error calculating complexity for ${document.uri.fsPath}`, error);
            return 1;
        }
    }
    ...
    private async getFileChangeFrequency(filePath: string): Promise<number> {
        try {
            ...
            if (fileAge < 24 * 60 * 60 * 1000) { // Less than 1 day old
                return 3;
            } else if (fileAge < 7 * 24 * 60 * 60 * 1000) { // Less than 1 week old
                return 2;
            }
