...
export class CodeAnalysisService implements ICodeAnalysisService, vscode.Disposable {
  ...
  public async analyzeWorkspace(): Promise<string[]> {
    ...
    for (const dir of commonDirs) {
      ...
      if (
        (await fileSystemService.exists(dirPath)) &&
        (await fileSystemService.isDirectory(dirPath))
      ) {
...
Don't explain the similarities, just list them.`;
  ...
  public async findDependencies(
    ...
    try {
      ...
      const includePattern = new vscode.RelativePattern(directoryPath, '**/*.{js,jsx,ts,tsx}');
      ...
      return Array.from(dependencies.values()).sort((a, b) => b.usageCount - a.usageCount);
    } catch (error) {
      this.context.loggingService.error(
        `Failed to find dependencies in directory: ${directoryPath}`,
        error
      );
      return [];
    }
