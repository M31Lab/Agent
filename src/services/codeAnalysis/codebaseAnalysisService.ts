...
export class CodebaseAnalysisService implements vscode.Disposable {
  ...
  public async analyzeWorkspace(): Promise<CodeAnalysisResult> {
    ...
    try {
      ...
      this.context.loggingService.info(
        `Workspace analysis complete: ${files.length} files, ${totalLines} lines`
      );
  ...
  public async searchCodebase(
    ...
    try {
      ...
      this.context.loggingService.info(
        `Codebase search for "${searchTerm}" found ${locations.length} results`
      );
