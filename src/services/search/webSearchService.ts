...
export class WebSearchService implements vscode.Disposable {
  ...
  private async searchWithBing(query: string, options: WebSearchOptions): Promise<SearchResult[]> {
    try {
      ...
      if (!apiKey) {
        throw new Error(
          'Bing API key not configured. Please set m31-agent.search.bingApiKey in your settings.'
        );
      }
      ...
      if (axios.isAxiosError(error) && error.response) {
        throw new Error(
          `Bing search error: ${error.response.status} - ${error.response.statusText}`
        );
      }
      throw error;
    }
  ...
  private async searchWithGoogle(
    ...
    try {
      ...
      if (!apiKey || !engineId) {
        throw new Error(
          'Google Search API not fully configured. Please set both m31-agent.search.googleApiKey and m31-agent.search.googleEngineId in your settings.'
        );
      }
      ...
      if (axios.isAxiosError(error) && error.response) {
        throw new Error(
          `Google search error: ${error.response.status} - ${error.response.statusText}`
        );
      }
      throw error;
    }
  ...
  private async searchWithDDG(query: string, options: WebSearchOptions): Promise<SearchResult[]> {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        throw new Error(
          `DuckDuckGo search error: ${error.response.status} - ${error.response.statusText}`
        );
      }
      throw error;
    }
  ...
  private async searchWithCustomEndpoint(
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        throw new Error(
          `Custom search error: ${error.response.status} - ${error.response.statusText}`
        );
      }
      throw error;
    }
  ...
  public async setProvider(provider: SearchProvider): Promise<void> {
    ...
    await vscode.workspace
      .getConfiguration('m31-agent.search')
  ...
  public async setCustomEndpoint(endpoint: string): Promise<void> {
    ...
    await vscode.workspace
      .getConfiguration('m31-agent.search')
      .update('customEndpoint', endpoint, vscode.ConfigurationTarget.Global);
  }
