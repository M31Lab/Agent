...
import { ContextTool, ContextToolType, ContextToolResult } from '../../models/contextTools';
    ...
    try {
      ...
      if (options.maxTokens && options.maxTokens > 0) {
        ...
        if (approxTokens > options.maxTokens) {
          ...
          markdown =
            markdown.substring(0, truncateLength) +
  ...
  public async executeProblemsTools(options: ProblemsToolOptions): Promise<ContextToolResult> {
    try {
      ...
      vscode.languages.getDiagnostics().forEach(([uri, diagnostics]) => {
        ...
        diagnostics.forEach((diagnostic) => {
          ...
          if (options.severity !== 'all') {
            const diagSeverity = this.getDiagnosticSeverityString(
              diagnostic.severity
            ).toLowerCase();
      ...
      if (diagnostics.length === 0) {
        ...
        // Group by file
        const problemsByFile = diagnostics.reduce(
          (groups, problem) => {
