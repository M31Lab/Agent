...
export class BrowserTestingService implements vscode.Disposable {
  ...
  private constructor(context: ExtensionContext) {
    ...
    this.browserService.onBrowserEvent((event) => {
      if (event.type === 'screenshotCaptured') {
        ...
        const activeExecution = Array.from(this.activeExecutions.entries()).find(
          ([_, data]) => data.browserId === event.sessionId
        );
  ...
  public async executeTestSuite(
    ...
    this.executeTestSteps(execution.id, browserId, testOptions).catch((error) => {
    ...
    try {
      ...
      for (let i = 0; i < suite.steps.length; i++) {
        ...
        if (
          ...
          (step.action === BrowserActionType.Navigate && options.waitForPageLoad)
        ) {
    ...
    for (const [executionId, data] of this.activeExecutions.entries()) {
      this.browserService.closeSession(data.browserId).catch((error) => {
        this.context.loggingService.error(
          `Error closing browser session for execution ${executionId}:`,
          error
        );
      });
    }
