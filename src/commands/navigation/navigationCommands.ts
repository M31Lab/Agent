...
export function registerNavigationCommands(
  ...
  registerCommand(context, 'm31-agent.navigateCodebase', async () => {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to process navigation query: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });

  // Search for files based on description
  registerCommand(context, 'm31-agent.findFile', async () => {
    const query = await vscode.window.showInputBox({
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to search for files: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });

  // Find code matching a description
  registerCommand(context, 'm31-agent.findCode', async () => {
    const query = await vscode.window.showInputBox({
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to search for code: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });

  // Create project overview
  registerCommand(context, 'm31-agent.projectOverview', async () => {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to generate project overview: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });
}
