  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.checkpoint.create', async () => {
      try {
        ...
        const checkpointId = await checkpointService.createCheckpoint(name, description);
    ...
    vscode.commands.registerCommand('m31-agent.checkpoint.compare', async () => {
      try {
        ...
        const fromCheckpoint = await vscode.window.showQuickPick(
          [{ label: 'Current Workspace', id: 'current' }, ...checkpoints],
          {
        ...
        if (diff.fileChanges.length === 0) {
          vscode.window.showInformationMessage(
            'No differences found between the selected checkpoints.'
          );
          return;
        }
        ...
        for (const change of diff.fileChanges) {
          ...
          if (change.oldContent !== undefined && change.newContent !== undefined) {
            const beforeUri = vscode.Uri.file(
              path.join(tempFolder, `${fileNameWithoutExt}.before${fileExt}`)
            );
            const afterUri = vscode.Uri.file(
              path.join(tempFolder, `${fileNameWithoutExt}.after${fileExt}`)
            );
        ...
        vscode.window.showErrorMessage(
          `Failed to compare checkpoints: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
