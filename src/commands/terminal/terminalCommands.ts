...
export function registerTerminalCommands(
  ...
  registerCommand(context, 'm31-agent.runCommand', async () => {
    const terminal =
      vscode.window.activeTerminal || vscode.window.createTerminal('M31 Agent Terminal');
    terminal.show();
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to process command request: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });

  // Execute a terminal command directly (requires user confirmation)
  registerCommand(context, 'm31-agent.executeCommand', async (commandToExecute?: string) => {
    if (!commandToExecute) {
  ...
  registerCommand(context, 'm31-agent.createTerminal', async () => {
  ...
  registerCommand(context, 'm31-agent.explainTerminalOutput', async () => {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to explain terminal output: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });
}
