...
export class CodeExecutionCommands {
  ...
  constructor(extensionContext: ExtensionContext, terminalService: TerminalService) {
  ...
  private registerCommands(): void {
    this._disposables.push(
      ...
      vscode.commands.registerCommand(
        'm31-agent.executeSelectedCode',
        this.executeSelectedCode.bind(this)
      ),
      vscode.commands.registerCommand(
        'm31-agent.createAndRunFile',
        this.createAndRunFile.bind(this)
      ),
      vscode.commands.registerCommand(
        'm31-agent.executeInTerminal',
        this.executeInTerminal.bind(this)
      )
    );
  ...
  private async createAndRunFile(
    content: string,
    language: string,
    runAfterCreate = true
  ): Promise<void> {
  ...
  private async executeInTerminal(text?: string): Promise<void> {
    if (!text) {
      ...
      if (editor) {
        const selection = editor.selection;
        text = selection.isEmpty ? editor.document.getText() : editor.document.getText(selection);
      }
    }
