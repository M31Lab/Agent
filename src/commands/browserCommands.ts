...
export function registerBrowserCommands(
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.browser.startSession', async () => {
      try {
        const headlessOption = await vscode.window.showQuickPick(['Headless', 'Visible'], {
        ...
        vscode.window.showErrorMessage(
          `Failed to start browser session: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
