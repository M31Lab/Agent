  ...
  const command = vscode.commands.registerCommand('m31-agent.configureSettings', async () => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Error configuring settings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });
...
async function configureAiParameters(context: ExtensionContext): Promise<void> {
  ...
  const options = ['Max Tokens', 'Temperature', 'Require Confirmation'];
    ...
    case 'Require Confirmation':
      {
        ...
        vscode.window.showInformationMessage(
          `Confirmation requirement ${newSetting ? 'enabled' : 'disabled'}`
        );
      }
      break;
  }
