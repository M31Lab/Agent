        ...
        vscode.commands.registerCommand('m31-agent.checkpoint.compare', async () => {
            try {
                ...
                const fromCheckpoint = await vscode.window.showQuickPick([
                    { label: 'Current Workspace', id: 'current', description: 'Compare from current workspace state' },
                    ...checkpoints
                ], {
                ...
                const toCheckpoints = fromCheckpoint.id === 'current'
                    ? checkpoints
                    : [
                        { label: 'Current Workspace', id: 'current', description: 'Compare to current workspace state' },
                ...
                // Open visualization
                await comparisonService.visualizeDiff(
                    fromCheckpoint.id,
                    toCheckpoint.id,
                    {
    ...
    disposables.push(
        vscode.commands.registerCommand('m31-agent.checkpoint.restoreWithOptions', async () => {
            try {
                ...
                if (success) {
                    vscode.window.showInformationMessage(`Checkpoint ${checkpoint.label} restored successfully`);
