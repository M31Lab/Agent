...
\`\`\``;
        ...
        try {
            const response = await this.apiClient.generateChatCompletion([
                {
                    ...
                    content: 'You are a helpful coding assistant that adds appropriate logging statements to code while preserving all functionality.'
                },
                {
                    role: ChatRole.User,
                    content: prompt
            ...
            throw new Error('Failed to add logs to code: ' + (error instanceof Error ? error.message : String(error)));
        }
    }
} 