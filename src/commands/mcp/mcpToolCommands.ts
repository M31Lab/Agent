...
export function registerMcpCommands(context: ExtensionContext): vscode.Disposable[] {
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.mcp.addTool', async () => {
      try {
        ...
        // Create the tool
        const toolId = await mcpService.createTool(name, description, author, [initialEndpoint]);
...
async function addEndpointToTool(context: ExtensionContext, toolId: string): Promise<void> {
  const mcpService = McpService.getInstance(context);
