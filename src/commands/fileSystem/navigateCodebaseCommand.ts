  ...
  const command = vscode.commands.registerCommand('m31-agent.navigateCodebase', async () => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to navigate codebase: ${error instanceof Error ? error.message : String(error)}`
      );
...
async function showProjectStructure(
  ...
  try {
    ...
    vscode.window.showErrorMessage(
      `Error showing project structure: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
function generateStructureMarkdown(structure: Record<string, unknown>, level: number = 0): string {
  ...
  for (const [name, info] of Object.entries(structure)) {
    ...
    if (isDirectory && (info as Record<string, unknown>).children) {
      result += generateStructureMarkdown(
        (info as Record<string, unknown>).children as Record<string, unknown>,
        level + 1
      );
    }
  }
...
async function aiPoweredFileSearch(
  context: ExtensionContext,
  dependencies: CommandDependencies
): Promise<void> {
  try {
...
Do not include any other text in your response, only valid JSON.`,
    ...
    try {
      ...
      if (selectedItem) {
        ...
        const fileUri = files.find(
          (f) => vscode.workspace.asRelativePath(f) === selectedItem.description
        );
    ...
    vscode.window.showErrorMessage(
      `Error in AI search: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
