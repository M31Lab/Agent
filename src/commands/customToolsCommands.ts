  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.customTools.create', async () => {
      try {
        ...
        const tool = await customToolsService.createTool(name, description, serverCode, endpoints, {
        ...
        vscode.window.showErrorMessage(
          `Failed to create custom tool: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.customTools.install', async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to install custom tool: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.customTools.list', async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to list custom tools: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
...
async function invokeToolEndpoint(
  customToolsService: CustomToolsService,
  tool: unknown
): Promise<void> {
  try {
    ...
    const endpointItems: EndpointQuickPickItem[] = tool.endpoints.map(
      (endpoint: CustomToolEndpoint) => ({
        label: endpoint.name,
