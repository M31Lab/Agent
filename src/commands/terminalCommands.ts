...
export function registerTerminalCommands(
    ...
    vscode.commands.registerCommand('m31-agent.terminal.executeCommand', async () => {
      try {
        const sessions = terminalService.getAllSessions().map((s) => ({
          label: s.id,
          description: s.status === 'running' ? `Running: ${s.currentCommand?.command}` : 'Idle',
        ...
        const requireConfirmation = await vscode.window.showQuickPick(['Yes', 'No'], {
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.terminal.cancelCommand', async () => {
      try {
        const sessions = terminalService
          .getAllSessions()
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.terminal.viewOutput', async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to view terminal output: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
