...
export function registerContextToolsCommands(
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.contextTools.url', async () => {
      try {
        ...
        const includeImages = await vscode.window.showQuickPick(['Yes', 'No'], {
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.contextTools.problems', async () => {
      try {
        const severity = await vscode.window.showQuickPick(['All', 'Error', 'Warning', 'Info'], {
        ...
        const result = await contextToolsService.executeProblemsTools({
          severity:
            severity === 'All' ? 'all' : (severity.toLowerCase() as 'error' | 'warning' | 'info'),
          maxProblems,
        });
        ...
        vscode.window.showErrorMessage(
          `Failed to fetch problems: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.contextTools.file', async () => {
      try {
        ...
        const useSelection = await vscode.window.showQuickPick(['Entire file', 'Specific lines'], {
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.contextTools.folder', async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to get folder content: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
  ...
  disposables.push(
    vscode.commands.registerCommand('m31-agent.contextTools.list', async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to list context tools: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    })
  );
