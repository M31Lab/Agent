...
export function registerSettingsCommands(
  ...
  registerCommand(context, 'm31-agent.configureSettings', async () => {
    const settings = await vscode.commands.executeCommand(
  ...
  // Configure API key
  registerCommand(context, 'm31-agent.configureApiKey', async () => {
  ...
  // Clear API key
  registerCommand(context, 'm31-agent.clearApiKey', async () => {
  ...
  // Select model
  registerCommand(context, 'm31-agent.selectModel', async () => {
    const models = [
      {
        label: 'OpenAI GPT-4o',
        value: 'openai/gpt-4o',
      ...
      },
      {
        ...
        description: 'Fast and economical general-purpose model',
      },
      {
    ...
    if (selectedModel) {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to change model: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  });
  ...
  registerCommand(context, 'm31-agent.resetSettings', async () => {
    const confirmed = await vscode.window.showWarningMessage(
  ...
  // Toggle telemetry
  registerCommand(context, 'm31-agent.toggleTelemetry', async () => {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to toggle telemetry: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });
}
