...
export function registerChatCommands(
  ...
  registerCommand(context, 'm31-agent.showChat', () => {
    chatPanelProvider.show();
  });

  // Quick chat with input box or selection
  registerCommand(context, 'm31-agent.quickChat', async () => {
  ...
  registerCommand(context, 'm31-agent.newChat', async () => {
    await chatPanelProvider.createNewSession();
    chatPanelProvider.show();
  });

  // Clear current chat
  registerCommand(context, 'm31-agent.clearChat', async () => {
  ...
  // Export chat history
  registerCommand(context, 'm31-agent.exportChat', async () => {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to export chat history: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });
}
