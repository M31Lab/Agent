...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  try {
    ...
    extensionContext.checkpointComparisonService = new CheckpointComparisonService(
      extensionContext
    );
    ...
    extensionContext.diagnosticsMonitoringService =
      DiagnosticsMonitoringService.getInstance(extensionContext);
    ...
    const updateDiagnosticsStatusBar = (): void => {
      ...
      if (counts.errors > 0) {
        ...
        diagnosticsStatusBar.backgroundColor = new vscode.ThemeColor(
    ...
    vscode.window.showErrorMessage(
      'Failed to activate M31-Agent extension. See output channel for details.'
    );
  }
}
