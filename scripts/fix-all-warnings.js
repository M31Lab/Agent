...
files.forEach((file) => {
  ...
  const unusedVarsOutput = execSync(
    `npx eslint --no-eslintrc --config .eslintrc.js --rule '@typescript-eslint/no-unused-vars:["error"]' ${file}`,
    { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] }
  ).toString();
  const unusedVarMatches = unusedVarsOutput.match(
    /'([^']+)' is (?:defined|assigned a value) but never used/g
  );
    ...
    unusedVarMatches.forEach((match) => {
      ...
      if (varNameMatch && varNameMatch[1] && !varNameMatch[1].startsWith('_')) {
        ...
        const varRegex = new RegExp(
          `\\b(const|let|var|function|class|interface|type|parameter|\\()\\s*(${varName})\\b(?!\\s*=\\s*_)`,
          'g'
        );
  ...
  const missingReturnOutput = execSync(
    `npx eslint --no-eslintrc --config .eslintrc.js --rule '@typescript-eslint/explicit-function-return-type:["error"]' ${file}`,
    { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] }
  ).toString();
  ...
  const anyTypesOutput = execSync(
    `npx eslint --no-eslintrc --config .eslintrc.js --rule '@typescript-eslint/no-explicit-any:["error"]' ${file}`,
    { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] }
  ).toString();
